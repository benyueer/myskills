...
### Installing community skills from the hub

```bash
hermes skills search QUERY           # find skills in the hub
hermes skills inspect ID             # preview before installing
hermes skills install ID             # install (may be blocked by security scan)
hermes skills install ID --force     # bypass security scan block
hermes skills tap add OWNER/REPO     # add a GitHub repo as skill source
```

Community skills (from skills.sh or taps) run a security scan on install. If
verdict is DANGEROUS (e.g. skill references config file paths like CLAUDE.md),
installation is blocked. Use `--force` to override after inspecting the skill
to verify it is safe. The `--yes` flag auto-confirms but does NOT bypass the
security block — only `--force` does.

If `hermes skills search` truncates identifiers in the table, use `hermes skills tap add OWNER/REPO` to add the source first, then install by short name.
